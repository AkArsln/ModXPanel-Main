...
def admin_dashboard():
    ...
    # Toplam kullanıcı
    toplam_kullanici = cur = conn.cursor()
    cur.execute('SELECT COUNT(*) FROM users').fetchone()[0]

    # Toplam mesaj
    toplam_mesaj = cur = conn.cursor()
    cur.execute('SELECT SUM(mesaj_sayisi) FROM messages').fetchone()[0] or 0
...
def announcements():
    ...
    # En son duyuruyu al
    announcement = cur = conn.cursor()
    cur.execute('SELECT * FROM announcements ORDER BY created_at DESC LIMIT 1').fetchone()
    ...
    if request.method == 'POST' and announcement and not already_read:
        cur = conn.cursor()
        cur.execute(
            'INSERT INTO announcement_reads (username, announcement_id) VALUES (%s, %s)',
...
def admin_duyurular():
    ...
    if request.method == 'POST':
        ...
        cur = conn.cursor()
    cur.execute('INSERT INTO announcements (title, content, created_at) VALUES (%s, %s, %s)',
                     (title, content, now))
...
def mesaj_girisi():
    ...
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute(
            'INSERT INTO messages (username, project, tarih, mesaj_sayisi) VALUES (%s, %s, %s, %s)',
...
def kullanici_chat():
    ...
    if request.method == 'POST':
        ...
        cur = conn.cursor()
    cur.execute('INSERT INTO admin_messages (gonderen, alici, mesaj, zaman) VALUES (%s, %s, %s, %s)',
